 ////// SOLUTIONS OF THE ASSIGNMENT QUESTIONS--3///////

//// Question--1////

 function sumThreeInteger(nums, target) {
  ...
  const nums = [-1, 2, 1, -4];
  const target = 1;
  const closestSum = sumThreeInteger(nums, target);
  console.log(closestSum); // Output: 2


  //// Question--2////

  function sumFourNums(nums, target) {
    nums.sort((a, b) => a - b); // Sort the array in ascending order
    const result = [];
  ...
  const target = 0;
  const quadruplets = sumFourNums(nums, target);
  ...
  function nextPermutationArray(nums) {
  ...
  // Example usage:
  const digits = [1, 2, 3];
  const result = incrementInteger(digits);
  ...
  function findMissingNumbers(nums, lower, upper) {
    const missingRanges = [];
  ...
  const result = findMissingNumbers(nums, lower, upper);
  console.log(result);
  ...
  function attendAllMeetings(intervals) {
  ...
  const result = attendAllMeetings(intervals);
  console.log(result);
  // Output: false
